...
Note that we do not have tests in place for the whole library at this stage. The best supported parts are `DMDA`, `DMStag`, `DMPlex`, `KSP`, `SNES`, `Vec`, and `Mat`. `DMPlex` supports the full FEM workflow: unstructured meshes (Gmsh), finite element spaces, pointwise residual/Jacobian callbacks (with ForwardDiff AD), boundary conditions, VTK output, and Lagrangian advection — see [`examples/ex62b.jl`](./examples/ex62b.jl) for a complete 2D/3D Stokes example. Other DM types (DMForest, DMNetwork, DMSwarm) do not yet have a high-level interface; users will have to rely on the low-level `LibPETSc` interface for those.
