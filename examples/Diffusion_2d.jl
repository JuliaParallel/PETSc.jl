...
function FormInitialGuess!(dm,x)
    ...
    x_Local = PETSc.
    x_array = PETSc.DMStagVecGetArray(dm,x);
    for i=1:length(x)
        x[i] = 0.50;
    end
end
...
dm  = PETSc.DMStagCreate2d(MPI.COMM_SELF,PETSc.DM_BOUNDARY_NONE,PETSc.DM_BOUNDARY_NONE,nx,nz,1,1,0,1,1,PETSc.DMSTAG_STENCIL_BOX,2);
