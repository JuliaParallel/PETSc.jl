...
"""
struct PetscArray{T,N,A<:AbstractArray{T,N},P} <: AbstractArray{T,N}
    data::A  # julia array (or view) that holds the data
...
Base.eltype(::Type{<:PetscArray{T}}) where {T} = T
...
# Type conversions
Base.convert(::Type{Array{T,N}}, A::PetscArray{T,N}) where {T,N} = convert(Array{T,N}, A.data)
